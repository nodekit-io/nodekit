...
Many objects in {NK} NodeKit emit events: a `net.Server` emits an event each time
...
to emit an `'error'` event.  Error events are treated as a special case in {NK} NodeKit.
