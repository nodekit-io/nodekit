...
(Note: All sockets in {NK} NodeKit set `SO_REUSEADDR` already)
...
user and used as a client (with `connect()`) or they can be created by {NK} NodeKit
...
simply might be too slow. {NK} NodeKit will internally queue up the data written to a
