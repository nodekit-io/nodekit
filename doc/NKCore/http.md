...
The HTTP interfaces in {NK} NodeKit are designed to support many features
...
In order to support the full spectrum of possible HTTP applications, {NK} NodeKit's
...
And {NK} NodeKit does not check whether Content-Length and the length of the body
...
`response.write()` is called, {NK} NodeKit assumes you're going to be streaming
...
{NK} NodeKit maintains several connections per server to make HTTP requests.
...
* Sending a 'Connection: keep-alive' will notify {NK} NodeKit that the connection to
...
Since most requests are GET requests without bodies, {NK} NodeKit provides this
...
socket to become free the socket is closed. This means that {NK} NodeKit's
...
marked so as to not keep the {NK} NodeKit app running.  However, it is
...
Note: {NK} NodeKit does not check whether Content-Length and the length of the body
which has been transmitted are equal or not.
...
For efficiency reasons, {NK} NodeKit normally buffers the request headers until you
